...
import {FormaAdquisicionInterface} from '../../interfaces/tablasGenerales/formas_adquisicion-tablasGenerales-interface';
...
export class FormasAdquisicionService {
  ...
  constructor(private http:HttpClient,private store: LocalService,) {
   ...
   getCombo(): Observable<FormaAdquisicionInterface[]>{
    this.insertHead()
    return this.http.post<FormaAdquisicionInterface[]>(`${environment.apiUrl}/pat/formas_adquisicion/combo`,this.httpOptions)
    .pipe(map(data => data));
   }

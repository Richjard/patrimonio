...
@Component({
  selector: 'app-empleados-form-tabla-generales',
  templateUrl: './empleados-form-tabla-generales.component.html',
  styleUrls: ['./empleados-form-tabla-generales.component.scss']
})
export class EmpleadosFormTablaGeneralesComponent implements OnInit {
...
public CargoList: DropDownListComponent;
  ...
  @Input("datosEmpleado") Empleado:EmpleadoInterface;
  @Input() op: string;//opcion de crud
  faSave = faSave;//icono guardar
  faFilter = faFilter;//icono filter
  faList=faList;
  @Output() devuelve_hijo:EventEmitter<EmpleadoInterface> = new EventEmitter<EmpleadoInterface>()

  //@Output() devuelve_hijo = new EventEmitter<LocalInterface>()
  @Output() cerrar_modal = new EventEmitter<string>()
  @ViewChild('EmpleadoForm',{static: true}) form: any;//fromulario
...
public GuardarEmpleado(): void {
  this.devuelve_hijo.emit(this.Empleado);

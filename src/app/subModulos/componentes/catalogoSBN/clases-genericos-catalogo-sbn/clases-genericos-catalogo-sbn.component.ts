...
//servicios
import {ClasesService} from '../../../servicios/catalogo_sbn/clases.service'
...
import {ClaseGenSyService} from '../../../servicios/catalogo_sbn/Clases.sy.service'
...
@Component({
  selector: 'app-clases-genericos-catalogo-sbn',
  templateUrl: './clases-genericos-catalogo-sbn.component.html',
  styleUrls: ['./clases-genericos-catalogo-sbn.component.scss']
})


export class ClasesGenericosCatalogoSBNComponent implements OnInit {
...
documentClick: EmitType<Object> = (e: MouseEvent) => {
  ...
  public ClaseGen:ClaseGenInterface;//formulario interfaz data
 ...
 public alertDlgBtnClick = () => {
     //eliminar registro seleccionado
      ...
      this.serviceCrud.delete(this.ClaseGen.iClaseGenId).subscribe((respon)=>{      
  ...
  @ViewChild('template',{static: true})
    ...
    public BtnVerClick: EmitType<object> = () => {  
      if(this.grid.getSelectedRecords().length>0) {  
           ...
           this.setClaseGenDatos(selectedrecords[0]['iClaseGenId'],selectedrecords[0]['cClaseGenCodigo'],selectedrecords[0]['cClaseGenDescripcion']);
...
constructor(private service: ClaseGenSyService, private serviceCrud:ClasesService){
  this.data = service;
}
...
setClaseGenDatos(iClaseGenId,cClaseGenCodigo,cClaseGenDescripcion){
  this.ClaseGen = {
    iClaseGenId :iClaseGenId,
    cClaseGenCodigo:cClaseGenCodigo,
    cClaseGenDescripcion :cClaseGenDescripcion
  };
}


public cerrar_ventana_modal(ClaseGen:ClaseGenInterface){  
  if(this.opcion==0){
    this.serviceCrud.crear(ClaseGen).subscribe((respon)=>{
    if(respon["validated"]==true)
     this.toastObj.show( { title: 'Éxito!', content: respon["mensaje"], cssClass: 'e-toast-success', icon: 'e-success toast-icons' });  
     this.grid.refresh();//refresescamos la grilñla  
    });    
  }else{
      this.serviceCrud.modificar(ClaseGen).subscribe((respon)=>{ 
      if(respon["validated"]==true)
      this.toastObj.show( { title: 'Éxito!', content: respon["mensaje"], cssClass: 'e-toast-success', icon: 'e-success toast-icons' });   
      this.grid.refresh();//refresescamos la grilñla 
      });

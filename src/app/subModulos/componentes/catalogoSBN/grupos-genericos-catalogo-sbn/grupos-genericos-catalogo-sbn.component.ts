...
//servicios
import {GruposService} from '../../../servicios/catalogo_sbn/grupos.service'
...
import {GrupoGenSyService} from '../../../servicios/catalogo_sbn/grupos.sy.service'
...
@Component({
  selector: 'app-grupos-genericos-catalogo-sbn',
  templateUrl: './grupos-genericos-catalogo-sbn.component.html',
  styleUrls: ['./grupos-genericos-catalogo-sbn.component.scss']
})



export class GruposGenericosCatalogoSBNComponent implements OnInit {
...
documentClick: EmitType<Object> = (e: MouseEvent) => {
  ...
  public GrupoGen:GrupoGenInterface;//formulario interfaz data
 ...
 public alertDlgBtnClick = () => {
     //eliminar registro seleccionado
      ...
      this.serviceCrud.delete(this.GrupoGen.iGrupoGenId).subscribe((respon)=>{      
      ...
      this.setGrupoGenDatos("autogenerado","","");
  ...
  @ViewChild('template',{static: true})
    ...
    public BtnVerClick: EmitType<object> = () => {  
      if(this.grid.getSelectedRecords().length>0) {  
           ...
           this.setGrupoGenDatos(selectedrecords[0]['iGrupoGenId'],selectedrecords[0]['cGrupoGenCodigo'],selectedrecords[0]['cGrupoGenDescripcion']);
...
constructor(private service: GrupoGenSyService, private serviceCrud:GruposService){
  this.data = service;
}
...
setGrupoGenDatos(iGrupoGenId,cGrupoGenCodigo,cGrupoGenDescripcion){
  this.GrupoGen = {
    iGrupoGenId :iGrupoGenId,
    cGrupoGenCodigo:cGrupoGenCodigo,
    cGrupoGenDescripcion :cGrupoGenDescripcion
  };
}


public cerrar_ventana_modal(grupoGen:GrupoGenInterface){  
  if(this.opcion==0){
    this.serviceCrud.crear(grupoGen).subscribe((respon)=>{
    if(respon["validated"]==true)
     this.toastObj.show( { title: 'Éxito!', content: respon["mensaje"], cssClass: 'e-toast-success', icon: 'e-success toast-icons' });  
     this.grid.refresh();//refresescamos la grilñla  
    });    
  }else{
      this.serviceCrud.modificar(grupoGen).subscribe((respon)=>{ 
      if(respon["validated"]==true)
      this.toastObj.show( { title: 'Éxito!', content: respon["mensaje"], cssClass: 'e-toast-success', icon: 'e-success toast-icons' });   
      this.grid.refresh();//refresescamos la grilñla 
      });

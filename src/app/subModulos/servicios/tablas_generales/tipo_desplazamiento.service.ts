...
import {TipoDesplazamientoInterface} from '../../interfaces/tablasGenerales/tipo_desplazamiento-tablasGenerales-interface';
...
export class TipoDesplazamientoService {
  ...
  constructor(private http:HttpClient,private store: LocalService,) {
   ...
   getCombo(): Observable<TipoDesplazamientoInterface[]>{
    this.insertHead()
    return this.http.post<TipoDesplazamientoInterface[]>(`${environment.apiUrl}/pat/tipo_desplazamiento/combo`,this.httpOptions)
    .pipe(map(data => data));
   }
